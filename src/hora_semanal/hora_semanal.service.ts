...
export class HoraSemanalService {
  constructor(
    ...
    private readonly repository: Repository<HoraSemanal>,
  ...
  async exportarCitasProfesor() {
    ...
    for (let index = 0; index < asesores.length; index++) {
      ...
      hoja
        .range('A1:B1')
        .merged(true)
        .value([['Nombre: ', null, null, null]]);
      hoja
        .range('C1:H1')
        .merged(true)
        .value([[element.nombre, null, null, null]]);
      ...
      hoja
        .range('C2:D2')
        .merged(true)
        .value([['Correo: ', null, null, null]]);
      hoja
        .range('E2:H2')
        .merged(true)
        .value([[element.correo, null, null, null]]);
      ...
      for (let index = 0; index < citas.length; index++) {
        ...
        hoja
          .range(`A${celdaInicio}:A${celdaFin}`)
          .merged(true)
          .value([[semana, null, null, null]]);
        hoja
          .range(`B${celdaInicio}:B${celdaFin}`)
          .merged(true)
          .value([[format(fechaCita, 'MMMM dd'), null, null, null]]);
        hoja
          .range(`C${celdaInicio}:C${celdaFin}`)
          .merged(true)
          .value([
            [
              elementCita.hora.toString().split(':')[0] +
                ':' +
                elementCita.hora.toString().split(':')[1],
              null,
              null,
              null,
            ],
        ...
        hoja
          .range(`D${celdaInicio}:D${celdaFin}`)
          .merged(true)
          .value([[elementCita.link, null, null, null]]);
        hoja
          .range(`E${celdaInicio}:E${celdaFin}`)
          .merged(true)
          .value([[elementCita.estadoCita.nombre, null, null, null]]);
        hoja
          .range(`F${celdaInicio}:F${celdaFin}`)
          .merged(true)
          .value([[elementCita.tipoCita.nombre, null, null, null]]);
        if (elementCita.estadoCita.id != 1) {
          hoja
            .range(`G${celdaInicio}:G${celdaFin}`)
            .merged(true)
            .value([[elementCita.equipocita.codigoEquipo, null, null, null]]);

          const estudiantes =
            resultadosAgrupados[elementCita.equipocita.codigoEquipo];
